...
export default function PatientConsultations({ consultationsPage, patient }: Readonly<PatientConsultationsProps>) {
    return (
        <aside className="hidden w-1/2 border-r border-gray-200 px-4 py-6 sm:px-6 lg:px-8 xl:block">
            <div className="flex flex-wrap justify-between">
                ...
                <PatientListConsultationButton name={patient.fullName ?? patient.username} />
            </div>
                        ...
                        <div
                            ...
                            {(() => {
                                switch (consultation.status) {
                                    case "SCHEDULED":
                                        return (
                                            <span className="inline-flex select-none items-center rounded-md bg-gray-50 px-2 py-1 text-xs font-medium text-neutral-600 ring-1 ring-inset ring-neutral-500/10">
                                                Scheduled
                                            </span>
                                        )
                                    case "ACCEPTED":
                                        return (
                                            <span className="inline-flex select-none items-center rounded-md bg-green-50 px-2 py-1 text-xs font-medium text-green-700 ring-1 ring-inset ring-green-600/20">
                                                Accepted
                                            </span>
                                        )
                                    case "IN_PROGRESS":
